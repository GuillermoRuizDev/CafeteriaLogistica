...
namespace Cafeteria.Application.Services
{
    public class WorkOrderService : IWorkOrderService
    {
        ...
        public async Task<IEnumerable<ListOrderViewModel>> GetWorkOrdersPage(int page, int pageSize)
        {
            ...
            var result = workOrder.Select(wo => new ListOrderViewModel()
            {
        ...
        {
            return (await _workOrderRepository.GetKardexAsync()).ToList().Select(kardex => new
            KardexDto(
                KardexId: kardex.KardexId,
                 TransactionDate: kardex.TransactionDate,
        ...
        private async Task UpdateKardex(Product product, double quantity)
        {
            ...
            foreach (var rawMaterial in rawMaterials)
            {
                ...
                kardex.Quantity = kardex.Quantity - (product.ProductRawMaterials.FirstOrDefault(r => r.RawMaterialId == rawMaterial.RawMaterialId).Quantity * (decimal)quantity);
